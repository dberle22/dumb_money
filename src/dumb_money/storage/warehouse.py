...
GOLD_TICKER_METRICS_MART_COLUMNS = [
    ...
    "return_6m",
    "return_1y",
    "primary_benchmark_return_1y",
    "secondary_benchmark_return_1y",
    "excess_return_primary_1y",
    "excess_return_secondary_1y",
    "annualized_volatility_1m",
    "annualized_volatility_3m",
    ...
    "peer_median_free_cash_flow_yield",
    "peer_median_market_cap",
    "selected_history_period_type",
    "selected_history_period_count",
    "latest_period_end_date",
    "latest_revenue_growth",
    "latest_eps_growth",
    "latest_gross_margin_value",
    "latest_operating_margin_value",
    "latest_free_cash_flow_margin",
    "total_score",
    "confidence_score",
    "market_performance_score",
    "market_performance_available_weight",
    "growth_profitability_score",
    "growth_profitability_available_weight",
    "balance_sheet_score",
    "balance_sheet_available_weight",
    "valuation_score",
    "valuation_available_weight",
    "sector_company_count",
    "sector_median_return_6m",
    "sector_median_return_1y",
    "sector_median_forward_pe",
    "sector_median_ev_to_ebitda",
    "sector_median_price_to_sales",
]

GOLD_SCORECARD_METRIC_ROWS_COLUMNS = [
    "scorecard_metric_row_id",
    "ticker",
    "score_date",
    "metric_id",
    "category",
...
CANONICAL_DUCKDB_TABLES: dict[str, WarehouseTableSpec] = {
    ...
    "gold_ticker_metrics_mart": WarehouseTableSpec(
        table_name="gold_ticker_metrics_mart",
        columns=tuple(GOLD_TICKER_METRICS_MART_COLUMNS),
        csv_dir_attr="marts_dir",
        csv_file_name="gold_ticker_metrics_mart/gold_ticker_metrics_mart.csv",
        description=(
            "Canonical gold-layer ticker metrics mart with one row per ticker and scoring date "
            "for reusable Sprint 6 section inputs."
        ),
    ),
    "gold_scorecard_metric_rows": WarehouseTableSpec(
        table_name="gold_scorecard_metric_rows",
        columns=tuple(GOLD_SCORECARD_METRIC_ROWS_COLUMNS),
        csv_dir_attr="marts_dir",
        csv_file_name="gold_scorecard_metric_rows/gold_scorecard_metric_rows.csv",
        description=(
            "Canonical gold-layer scorecard metric rows with one row per ticker, score date, "
            "and metric id for reusable score transparency sections."
        ),
    ),
